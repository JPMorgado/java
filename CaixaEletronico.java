...
public class CaixaEletronico {
	...
	public static void mostrar_estatisticas(int notas[][],int notas_final[][])throws IOException{
		...
		for(int i=0;i<3;i++){
			if(i==0){
				System.out.printf("Maior valor de saque banco é %d\n",est[i][menu-1]);
			}else if(i==1){
				System.out.printf("Menor valor de saque banco é %d\n",est[i][menu-1]);
			}else if(i==2){
				est[i][menu-1]=est[i][menu-1]/cta[0][menu-1];
				System.out.printf("Média do banco %d\n",est[i][menu-1]);
			}
		}
		for(int i=0;i<6;i++){
			int var=notas[1][i]-notas_final[1][i];
			var=notas[1][i]-var;
			System.out.printf("Valor de notas %d é: %d\n",notas[0][i],var);
		}
	}
